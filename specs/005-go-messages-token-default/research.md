...
Use `4096` when `max_tokens` is absent and preserve any explicit value.
...
present. The 64-token probe was diagnostic only; 4096 is selected as a useful
bounded application default and remains user-overridable through the existing
configuration path.

**Residual verification**: a real, non-financial Go Messages smoke with 4096
